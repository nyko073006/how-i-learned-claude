...
### Kontext bei Bedarf
Wenn der Nutzer fragt "Was ist gerade aktuell?", "Wo war ich stehen geblieben?" oder aehnliches: `Index.md` gibt den Schnell-Ueberblick. Fuer mehr Tiefe: letzte 2-3 Daily Notes in `05 Daily Notes/` und die aktiven Projekt-Dateien in `02 Projekte/`. Aus den Daily Notes nur die Sektionen mit dem Tag des aktuellen Projekts lesen (z.B. `grep -n '#<projekt-tag>'`), nicht die ganze Datei. Das spart Token, weil parallele Projekte am selben Tag in dieselbe Datei schreiben.
...
6. **Immer projektbezogen.** Jede Sektion gehoert zu genau einem Projekt (`02 Projekte/`) oder Bereich (`03 Bereiche/`) und traegt dessen Tag plus Wikilink auf Hub oder Uebersicht. Sektionen ohne Projekt- oder Bereichsbezug gibt es nicht; was keinem zugeordnet werden kann, kommt in `01 Inbox/`. Inhalt nur, was die naechste Session dieses Projekts braucht. Grund: Token sparen. Eine KI liest beim Start nur die Sektionen ihres Projekts, nie die ganze Daily Note.
